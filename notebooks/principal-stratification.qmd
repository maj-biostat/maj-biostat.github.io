...
The likelihood decomposes into an S-model (a principal strata model given the covariates) and a Y-model (an outcome model given the stratum, covariates and treatment).
This yields the full likelihood:
...
where $f_{i,sz} = \text{Pr}(Y_i | S_i = s, Z_i, \mathbf{X}_i, \theta)$ and $\pi_{i,s} = \text{Pr}(S_i = s | \mathbf{X}_i, \theta)$ and where the $c,n,a,d$ denote the compliers, never-takeres, defiers, compliers.
...
We can compute the posterior expected mean for each arm and each strata as follows:
