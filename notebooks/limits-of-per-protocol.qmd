...
In RCTs, a traditional per-protocol analyses will exclude those patients that are not completers and that deviate in some unacceptable way from the protocol and then run the primary analysis unchanged.
This can be problematic in that it violates the randomisation principle, the treatment groups can become unbalanced in either known or unknown covariates, selection bias can arise, effect estimates may be biased and external validity threatened.

The following provides an example of the issue.
