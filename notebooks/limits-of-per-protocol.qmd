...
In RCTs, traditional per-protocol analyses exclude those patients that are not completers and that deviate in some unacceptable way from the protocol.
Such an approach can be problematic in that it violates the randomisation principle, the treatment groups can become unbalanced in either known or unknown covariates, selection bias can arise, effect estimates may be biased and external validity threatened.

The following provides an example via a hypothetical anntihypertensive drug trial.
We compare a new antihypertensive (A) to a standard treatment (B) for lowering blood pressure in patients with existing hypertension.
...
For this example, assume that A causes more side effects than B and at 3 months, 20% of patients stop taking A due to side effects and 4% stop taking B for a side effect intercurrent event, which we interpret here as a protocol deviation (although in reality who knows whether this would actually be consider to be a protocol deviation).
Additionally assume that those who experience side effects (and therefore dropout) tend to have more severe hypertension and would have shown less improvement in blood pressure status irrespective of the treatment they received.

The traditional per-protocol analysis exclude those that deviate from the protocol and would therefore have around 400 patients in A and 480 patients in B.
...
The plot below shows the results, specifically, the distribution of the estimated treatment effect (risk difference) which is known to have a true value of 0.2 in favour of the new drug (A).

Under the ITT analysis (where we ignore the fact that the ICE occurred and simply proceed to analyse all the data) the long-run expected value for the MLE estimate of the treatment effect aligns with the true value of 0.2.
...
This final approach is aligned with a G-computation perspective and similar to the results from that would be produced from an inverse probability of censoring weighting scheme.
In both cases, we are essentially, producing a re-weighted estimate of the effect.
...
The difference between the two per-protocol approaches can be emphasised by looking at the distribution of the percentage differences between the ITT effect and the PP effect.
...
d_fig[variable == "pct_diff_1", variable := "Pct diff (traditional PP vs ITT)"]
d_fig[variable == "pct_diff_2", variable := "Pct diff (g-computation PP vs ITT)"]

d_fig[, variable := factor(variable, levels = c(
  "Pct diff (traditional PP vs ITT)", 
  "Pct diff (g-computation PP vs ITT)"
))]
