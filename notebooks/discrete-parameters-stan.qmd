...
However, sometimes interested in discrete parameters.

Given that HMC run on log-probabilities, then as long as we can increment the log-probability associated with our model, we can code up discrete parameters.
This typically requires us to marginalise out the discrete parameter(s) of interest and then increment to the log-density by that marginal density.

Ben Lamberts example:

Consider a series of experiments where get someone to flip a coin $n$ times, using the same coin across all the experiments.
...
Both $\theta$ and $n$ are unknown to us, $\theta$ is continuous, but $n$ is discrete and so not directly amenible to inference in stan.

Assume the following data is observed for $X_k = (2, 4, 3, 3, 3, 3, 3, 3, 4, 4)$ for $K = 10$ experiments and that we will adopt independent priors on $\theta$ and $n$, namely:
...
To proceed we first write down the joint posterior of $\theta$ and $n$, i.e. $\text{Pr}(\theta, n | X)$ and then work towards marginalising out $n$.
